...
if(!IS_AJAX) { echo 'Khong duoc phep truy cap.'; }   //Check Ajax
else { 
    if (isset($_SESSION[mu_username])) {
        include_once("config_support.php");
        if ( $user_support == 1 )
        {
            $unread = 0;
            ...
            if($is_bqt == 1) echo '{"unread":'. $unread .',"notans":'. $support_notans .'}';
            else echo '{"unread":'. $unread .'}';
        }
    }
}   // End Check Ajax
?>