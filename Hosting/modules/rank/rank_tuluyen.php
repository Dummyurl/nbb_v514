...
if ( !isset($_SESSION['mu_username']) ) {
	...
	*/
	$file_host = "data/top_tuluyen.txt";
	...
	if ($time >= ($time_host+1*60*60) || $time_host > $time || date("d", $time) != date("d", $time_host) )
	{
		...
		$getcontent_url = $server_url . "/view_toptuluyen.php";
		...
		if ( !empty($show) )
		{
            ...
            if($info == "OK") {
                $toptuluyen = read_TagName($show, 'toptuluyen', 1);
                //Ghi vào file
	...
	while (!feof($fopen_host)) {
		$toptuluyen_load = fgets($fopen_host);
		
		if ( strlen($toptuluyen_load) < 50 )
		{
			$time_top = date("H:i A d/m/Y", intval($toptuluyen_load));
		}
		else {
			$toptuluyen_arr = json_decode($toptuluyen_load, true);
		}
	}
	
	fclose($fopen_host);

	$page_template = 'templates/rank/rank_tuluyen.tpl';
}
}
?>