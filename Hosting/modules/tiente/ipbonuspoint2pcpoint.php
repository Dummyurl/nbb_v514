...
{
	$action = $_POST['action'];
	if ($action == 'ipbonuspoint2pcpoint')
	{
		...
		elseif (!preg_match("/^[0-9]*$/i", $ipbonuspoint))
			{
				$notice = "Dữ liệu lỗi - IP Bonus Point chỉ được sử dụng số (1-9).";
			}
		elseif (empty($pass2))
		{
		...
		elseif ($_SESSION['IPBonusPoint'] < $ipbonuspoint) {
            ...
            $getcontent_data = array(
                ...
                'pagesv'	=>	'sv_tiente_ipbonuspoint2pcpoint',
...
$page_template = "templates/tiente/ipbonuspoint2pcpoint.tpl";
?>