...
if (isset($_POST['action']))
{
	...
	{
		$ans = $_POST['ans'];
		$ans_verify = $_POST['ans_verify'];
		
		$leng_ans = strlen($ans);
		
		if( $sendsv === false ) { $notice = "Tốc độ xử lý của bạn quá nhanh, vui lòng chờ vài giây rồi tiếp tục thực hiện."; }
    	elseif(empty($ans)) {
			$notice = "Chưa nhập Câu trả lời Bí mật mới";
		}
		elseif(empty($ans_verify)) {
			$notice = "Chưa xác minh Câu trả lời Bí mật mới";
		}
		...
		elseif (preg_match("/[^a-zA-Z0-9_$]/", $ans_verify))
			{
				$notice = "<font size='4' color='red'>Dữ liệu lỗi - Xác minh Câu trả lời bí mật chỉ được sử dụng kí tự a-z, A-Z, số (1-9) và dấu _.</font>";
			}
		elseif ($ans != $ans_verify)
			{
				$notice = "<font size='4' color='red'>Dữ liệu lỗi - Câu trả lời bí mật mới và Xác minh Câu trả lời bí mật mới không trùng khớp.</font>";
			}
		elseif($leng_ans<4)
			{
				$notice = "<font size='4' color='red'>Dữ liệu lỗi - Câu trả lời bí mật mới phải có ít nhất 4 kí tự.</font>";
			}
		else {
...
$page_template = "templates/acc_manager/changeans.tpl";
}
?>