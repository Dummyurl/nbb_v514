...
include_once('config/config_reset_over.php');
...
//Ghi vào Log nhung nhan vàt Reset
        ...
        $log_vpoint = $info_log[2];
        $log_price = " - ";
        $log_Des = "<b>$name</b> Reset OVER khi ". $row[0] ." Level/". $resetold ." Reset, Relife $relife. Nhận $uythacpoint Điểm Ủy Thác.<br />$point_msg .<br /> $danhvong_msg";
        $log_time = $timestamp;
...
//End Ghi vào Log nhung nhan vàt Reset tren 0 lan
    ...
    <messenge>$name <strong>Reset OVER</strong> ở lần Reset thứ <strong>$resetup</strong> thành công!<hr />Nhận được : <strong>$uythacpoint Điểm Ủy Thác</strong>.<hr />$danhvong_msg";
    if($Use_TuLuyen == 1) {
