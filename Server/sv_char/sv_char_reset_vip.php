...
if($use_gioihanrs[$thehe] == 1) {
    ...
    $gioihanrs_extra = $gioihanrs;
    if(isset($gioihanrs) && isset($use_overrs[$thehe]) && $use_overrs[$thehe] == 1) {
        $gioihanrs_extra = $gioihanrs + $overrs_rs[$thehe];
    }
    
    if(isset($gioihanrs) && $ResetDay >= $gioihanrs) {
        if( $ResetDay < $gioihanrs_extra) {
            if($tiente != 'gcoin') {
                echo "Để Reset vượt mức bạn phải Reset bằng Gcoin"; exit();
            } else {
                $gcoin_extra = $overrs_gcoin[$thehe];
            }
        } else {
...
if(file_exists('config/config_sendmess.php')) {
    ...
    if($Use_SendMess_RSVIP == 1) {
        ...
        $mess_send = '['. $thehe_name. '] '. $name .' Reset VIP lần thứ '. $resetup;
...
//Ghi vào Log
        ...
        $log_Des = "<b>$name</b> Reset VIP lần thứ <b>$resetup</b> khi ". $row[0] ." Level/". $resetold ." Reset, Relife $relife.<br />$point_msg .<br /> $danhvong_msg";
        $log_time = $timestamp;
...
//End Ghi vào Log
    ...
    <messenge>$name Reset VIP lần thứ $resetup thành công!<hr />$danhvong_msg";
    if($Use_TuLuyen == 1) {
    ...
    if(strlen($castle_onwer_notice) > 0) {
        $reponse .= "<br />". $castle_onwer_notice;
    }
