	...
	if ( empty($reponse) ) {
        ...
        if($info == "Error") {
            ...
            if(strlen($giftcode) == 0) {
                ...
                $name_query = "SELECT dulieu1 FROM SMS WHERE KeyXuLy='GIFTCODE_RS' AND acc='$taikhoan' AND Code='$code'";
                $name_result = $db->Execute($name_query);
                    check_queryerror($name_query, $name_result);
                $name_fetch = $name_result->FetchRow();
                $name = $name_fetch[0];
                
                $giftcode_insert_query = "INSERT INTO GiftCode (gift_code, acc, name, type, gift_time, ngay, status) VALUES ('$giftcode', '$taikhoan', '$name', 1, $timestamp, '".date("Y-m-d",$timestamp)."', 1)";
                $giftcode_insert_result = $db->Execute($giftcode_insert_query);
                $content = "Ma GiftCode Reset cua nhan vat $name : $giftcode";
             	//Delete Data
