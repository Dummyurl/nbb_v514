	...
	if ( empty($reponse) ) {
        ...
        if($info == "Error") {
            ...
            if(strlen($giftcode) == 0) {
                ...
                $giftcode_unused_del_query = "DELETE GiftCode WHERE type=2 AND gift_timeuse IS NULL AND acc='$taikhoan'";
                ...
                $giftcode_insert_query = "INSERT INTO GiftCode (gift_code, acc, type, gift_time, ngay, status) VALUES ('$giftcode', '$taikhoan', 2, $timestamp, '".date("Y-m-d",$timestamp)."', 1)";
                $giftcode_insert_result = $db->Execute($giftcode_insert_query);
                $content = "Ma GiftCode Tuan cua tai khoan $taikhoan : $giftcode";
             	//Delete Data
