    	...
    	case 'daily':
            ...
            for($i=1;$i<count($thehe_choise);$i++) {
                if(strlen($thehe_choise[$i]) > 1) {
                    ...
                    $top_daily_query = "SELECT TOP 20 TopResetScore.name, reset_score, Class, thehe FROM TopResetScore JOIN MEMB_INFO ON TopResetScore.acc collate DATABASE_DEFAULT =MEMB_INFO.memb___id collate DATABASE_DEFAULT AND thehe=". $i ." AND [day]=". $day ." AND [month]=". $month ." AND [year]=". $year ." JOIN Character ON TopResetScore.name collate DATABASE_DEFAULT = Character.Name collate DATABASE_DEFAULT ". $type_query ." ORDER BY reset_score DESC, Resets_Time";
                    ...
                    $top_daily_query = "SELECT TOP 20 TopResetScore.name, reset_score, Class, thehe FROM TopResetScore JOIN MEMB_INFO ON TopResetScore.acc collate DATABASE_DEFAULT =MEMB_INFO.memb___id collate DATABASE_DEFAULT AND thehe=". $i ." AND [day]=". $day_yesterday ." AND [month]=". $month_yesterday ." AND [year]=". $year_yesterday ." JOIN Character ON TopResetScore.name collate DATABASE_DEFAULT = Character.Name collate DATABASE_DEFAULT ". $type_query ." ORDER BY reset_score DESC, Resets_Time";
                    $top_daily_result = $db->Execute($top_daily_query);
                        check_queryerror($top_daily_query, $top_daily_result);
                    while($top_daily_fetch = $top_daily_result->FetchRow()) {
                        $TopResetScore[$action][$type][$date_yesterday][$i][] = array(
    	...
    	case 'week':
            ...
            for($i=1;$i<count($thehe_choise);$i++) {
                if(strlen($thehe_choise[$i]) > 1) {
                    ...
                    $top_week_query = "SELECT TOP 20 TopResetScore.name, SUM(reset_score) AS ResetScoreWeek, Class, thehe FROM TopResetScore JOIN MEMB_INFO ON TopResetScore.acc collate DATABASE_DEFAULT =MEMB_INFO.memb___id collate DATABASE_DEFAULT AND thehe=". $i ." AND [week]=". $week_now ." JOIN Character ON TopResetScore.name collate DATABASE_DEFAULT = Character.Name collate DATABASE_DEFAULT ". $type_query ." GROUP BY TopResetScore.name, Class, Thehe, Resets_Time ORDER BY ResetScoreWeek DESC, Resets_Time";
                    ...
                    $top_week_query = "SELECT TOP 20 TopResetScore.name, SUM(reset_score) AS ResetScoreWeek, Class, thehe FROM TopResetScore JOIN MEMB_INFO ON TopResetScore.acc collate DATABASE_DEFAULT =MEMB_INFO.memb___id collate DATABASE_DEFAULT AND thehe=". $i ." AND [week]=". $week_before ." JOIN Character ON TopResetScore.name collate DATABASE_DEFAULT = Character.Name collate DATABASE_DEFAULT ". $type_query ." GROUP BY TopResetScore.name, Class, Thehe, Resets_Time ORDER BY ResetScoreWeek DESC, Resets_Time";
                    $top_week_result = $db->Execute($top_week_query);
                        check_queryerror($top_week_query, $top_week_result);
                    while($top_week_fetch = $top_week_result->FetchRow()) {
                        $TopResetScore[$action][$type][$week_before][$i][] = array(
    	...
    	case 'month':
            ...
            for($i=1;$i<count($thehe_choise);$i++) {
                if(strlen($thehe_choise[$i]) > 1) {
                    ...
                    $top_month_query = "SELECT TOP 20 TopResetScore.name, SUM(reset_score) AS ResetScoreMonth, Class, thehe FROM TopResetScore JOIN MEMB_INFO ON TopResetScore.acc collate DATABASE_DEFAULT =MEMB_INFO.memb___id collate DATABASE_DEFAULT AND thehe=". $i ." AND [month]=". $month ." AND [year]=". $year ." JOIN Character ON TopResetScore.name collate DATABASE_DEFAULT = Character.Name collate DATABASE_DEFAULT ". $type_query ." GROUP BY TopResetScore.name, Class, Thehe, Resets_Time ORDER BY ResetScoreMonth DESC, Resets_Time";
                    ...
                    $top_month_query = "SELECT TOP 20 TopResetScore.name, SUM(reset_score) AS ResetScoreMonth, Class, thehe FROM TopResetScore JOIN MEMB_INFO ON TopResetScore.acc collate DATABASE_DEFAULT =MEMB_INFO.memb___id collate DATABASE_DEFAULT AND thehe=". $i ." AND [month]=". $month_before ." AND [year]=". $year_before ." JOIN Character ON TopResetScore.name collate DATABASE_DEFAULT = Character.Name collate DATABASE_DEFAULT ". $type_query ." GROUP BY TopResetScore.name, Class, Thehe, Resets_Time ORDER BY ResetScoreMonth DESC, Resets_Time";
                    $top_month_result = $db->Execute($top_month_query);
                        check_queryerror($top_month_query, $top_month_result);
                    while($top_month_fetch = $top_month_result->FetchRow()) {
                        $TopResetScore[$action][$type][$month_before][$i][] = array(
