...
$query = "select gcoin,WCoinP from MEMB_INFO WHERE memb___id='$login'";
$result = $db->Execute( $query ) or die("Loi query: $query");
$row = $result->fetchrow();


$gcoin_change = $row[0] - $gcoin;
$WCoinP_change = $row[1] + $gcoin;

if( $gcoin_change < 0 ){
echo "Bạn đang có $row[0] Gcoin. Bạn không thể đổi $gcoin Gcoin sang WCoinP."; exit(); }

$general = "UPDATE MEMB_INFO SET gcoin='$gcoin_change', WCoinP = '$WCoinP_change' WHERE memb___id='$login'";
